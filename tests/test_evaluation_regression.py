...
实现状态：Current
被测代码：evaluation/fixture_builder.py、evaluation/replay.py、evaluation/regression.py
"""
...
from evaluation.fixture_builder import build_fixture
from evaluation.regression import compare_fixture, main
from evaluation.replay import materialize_fixture

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = ROOT / "evaluation" / "fixtures" / "math-sample-v1"
...
def test_regression_cli_uses_stable_success_and_contract_exit_codes(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [
        "evaluation.regression", "--fixture", str(FIXTURE),
    ...
    monkeypatch.setattr(sys, "argv", [
        "evaluation.regression", "--fixture", str(tmp_path / "missing"),
        "--actual", str(actual), "--output", str(output),
    ])
