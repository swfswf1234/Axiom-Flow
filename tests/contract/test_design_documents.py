...
from axiom_flow.application.evaluation_analysis import (
    FORMAL_MAX_MODEL_CALLS,
    FORMAL_MINIMUM_AVERAGE,
    FORMAL_PAGE_COUNT,
...
def test_web_design_tracks_exact_main_view_ids():
    ...
    assert parser.views == {"pages", "knowledge", "workbook", "graph", "evaluation"}
...
def test_evaluation_design_tracks_scorecard_contract():
    ...
    assert f"固定要求 {FORMAL_PAGE_COUNT} 页" in content
    assert f"最大模型调用数为 {FORMAL_MAX_MODEL_CALLS}" in content
    assert f"平均分至少为 {FORMAL_MINIMUM_AVERAGE}" in content
    for dimension in VALID_SCORE_DIMENSIONS:
