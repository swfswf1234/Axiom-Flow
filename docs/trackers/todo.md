...
| V2-014 | Feature | P1 | Completed | 模型模式与 MinerU 移交（REQ-044）（需求方：QED-Engine 根仓库 LLM 网关与模型管理轮，设计：[model-mode-config](../design/model-mode-config.md)）：① `.env` 调整（`QED_API_SELECT=local` 默认、`API_KEY` 唯一、`QED_LLM_GATEWAY_URL`、`QED_DB_*`）；② `src/axiom_flow/config.py` + `llm_client.py` 双模式（local 直连 qwen-vl / qed-engine 经 8900 `/llm/vision` 调 MinerU，**MinerU 仅经网关可达**）；③ `scripts/axiom_flow_service.py --mode local/qed-engine`（重启可换模式）；④ **MinerU 编排移交**：compose.yaml/infra-*.ps1/docker/Dockerfile 迁出至根仓库 `scripts/image-model/`，本地 mineru 直连调用删除；⑤ local 模式直连调用记录写 `qed_llm_calls`（`service=axiom_flow`） | **MODEL-001 轮完成（2026-08-21）**：config.py(19 tests)/llm_client.py(16 tests + fallback 重构)/service --mode(35 tests)/文档同步+契约修复；T5 本地冒烟通过；T6 网关冒烟通过（call_id=17）；T7 跨项目通过；提交 `fcb9eb9..c212e8e`（9 commits）；门禁 179 passed + ruff 全绿 |
