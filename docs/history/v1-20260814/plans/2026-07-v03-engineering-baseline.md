...
归档判定：Retain；发布后保留标签和发布锚点作为里程碑证据。

## 目标与成功标准

在已完成的 v0.3 实现上执行发布门禁，将通过本地关闭门禁验证的 `main` 提交标记为
`v0.3.0`。成功标准是 REG-001 已关闭、目标提交的本地关闭门禁全部通过，且远端标签准确
指向该提交。

## 范围与非目标

本计划只负责核对发布目标、执行本地关闭门禁、复核完整差异和创建标签。不修改生产代码、测试、
...
- 发布人可核对 `origin/main`、本地门禁输出和远端标签目标。
...
- [ ] 冻结唯一预定发布提交及其回滚锚点。
- [ ] 确认该提交已推送到 `origin/main`，同一提交的全部本地门禁通过。
...
分层测试变更尚在未提交工作树中，`HEAD` 虽与 `origin/main` 一致，但还不存在可冻结并通过本地
门禁的唯一发布提交。

恢复条件：独立 D 类计划完成 `DATA-002` 的备份、清理、回滚验证和关闭；当前实现形成唯一提交、
合入 `main` 并通过适用本地门禁后，才恢复本计划并冻结发布提交。

责任位置：`DATA-002` 的责任位置是后续独立数据操作计划；提交冻结、本地门禁和标签复核仍由
`REL-001` 负责。
...
`git status`、`origin/main` 和目标提交一致，并人工核对本地门禁输出与提交 SHA 一致。
标签推送后复查远端 `v0.3.0` 只指向已验证提交。

## 回滚

本地关闭门禁未通过时不得创建标签。标签创建前的远端推送通过新增修复提交回滚，不改写 `main` 历史；
标签目标错误时停止发布，删除错误标签属于新的 D 类操作，必须先核对本地和远端精确目标。

## 关闭与归档

完成发布后写入 `关闭结果：Achieved`，记录提交、标签和门禁证据，并按 ADR 0013 移入
