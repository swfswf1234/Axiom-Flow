...
普通分支或 `main` 的源码提交与推送是原任务的交付步骤，不单独建立 D 类计划。日常小规模数据
与发布操作（单次命令级）不建立独立计划，由 git 提交与标签、台账证据列留痕；正式发布、受保护
环境、大规模数据操作和需要独立回滚的发布动作仍必须使用 D 类计划。
...
| 类别 | 定义 | 示例 |
| --- | --- | --- |
| 主线 | 大类目标，由活跃计划或大任务承载，聚合多条支线 | 文档对齐轮（DOCS-001）、端口与数据目录迁移（ALN-007） |
| 支线 | 主线任务的细则，可独立完成（请求类与期次计划为主） | 跨项目请求 ALN-xxx、迁移工作项 |
| 长期 | 持续存在、无单一终态，随项目演进持续生效 | 生产运维基线（OPS-001）、能力路线图方向 |
...
- B、C 类必须在 `docs/plans/` 建立独立计划；D 类按[任务分类](#任务分类)的规模分级执行（日常
  小操作 git 留痕，正式发布/受保护环境/大规模数据操作仍要求 D 类计划）。跨模块、跨会话或需要
  显式回滚的非平凡 A 类也必须建立计划；简单措辞、链接和局部无行为修正由差异、验证与提交记录
  承接。
...
- todo 使用稳定 ID 保存全部未关闭 Plan、Defect、Gap 和 Candidate。候选具备范围、前置条件、
  ...
  互相引用。
- 跨项目协作请求以 ALN- 承接并在 todo 登记，按[跨项目协作流程](cross-project-collaboration.md)
  执行评审、计划与回执，遵守本仓库门禁。
