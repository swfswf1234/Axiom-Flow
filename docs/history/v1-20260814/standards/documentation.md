...
本标准规定本仓库每类文档保存什么事实、采用什么元数据，以及何时归档或删除。具体任务状态查
...
本仓库是 QED-Engine 的子项目，文档体系独立于根仓库；根仓库（QED-Engine）与 QED-Tracker 的
文档规范不适用于本仓库。跨项目文档协作（端口、数据目录、配置直读、接口对接等）按
[跨项目协作流程](cross-project-collaboration.md)执行，本仓库不直接修改其他仓库文件。
...
| 根 `AGENTS.md` | Agent 执行总纲：项目目标、任务路由、问题定位与完成检查。只指引，不保存正文事实。 |
