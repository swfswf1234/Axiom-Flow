...
ADR 使用全局四位编号并永久保留在本目录，不按年份或产品版本拆分。版本只记录为决策阶段；
完整规则见 [ADR 治理规范](../standards/adr-governance.md)，新文件使用 [ADR 模板](../templates/adr.md)。

下一个可用编号：0013

| ADR | 标题 | 领域 | 决策阶段 | 状态 | 取代关系 |
| --- | --- | --- | --- | --- | --- |
| [`0001`](0001-local-first-and-storage.md) | 本地优先与运行数据存储 | 数据与持久化 | v0.2 | Superseded | 被 0005 取代 |
| [`0002`](0002-parser-routing-and-provider-boundary.md) | 分层解析路由与供应商适配边界 | 解析与评测 | v0.2 | Superseded | 被 0010 取代 |
| [`0003`](0003-excel-publish-source-of-truth.md) | Excel 显式发布工作流 | 审阅与发布 | v0.2 | Accepted | — |
| [`0004`](0004-v02-http-api-boundary.md) | v0.2 HTTP API 与静态工作台边界 | API 与任务 | v0.2 | Superseded | 被 0006 取代 |
...
登记表按编号排序，每份 ADR 只登记一次。领域、阶段或状态变化必须同时修改 ADR 元数据和本表；
Superseded 与 Rejected ADR 不移动、不删除。
