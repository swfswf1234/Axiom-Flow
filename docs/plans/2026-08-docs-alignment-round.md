...
| W1 | standards/：新增 governance-contract.md（守护面按本仓库契约测试清单裁剪）与 cross-project-collaboration.md（承接侧流程）；task-lifecycle 增任务层级节、D 类规模分级、C 类措辞；adr-governance 增登记时机/章节/审查留痕节（领域保留 5 值）；documentation 增 AGENTS 行与子项目独立体系段；index 扩 7 行表；守护测试先行 | 待开始 |
| W2 | architecture/：新增 project-status.md（当前主线、各域状态、维护规则、进场先读）；index 改状态列式；code-map 复核 | 待开始 |
