...
环境：从仓库根向上查找 `.env`，setdefault 注入 `AXIOM_*` 与供应商 key（已有环境不覆盖），
standalone 启动不缺凭据。接口契约（含 8900 控制中心接入方式）见
...
def _load_env(start: Path) -> None:
    """从 start 向上查找 `.env`，注入 `AXIOM_*` 与供应商 key（已有环境变量不覆盖）。
    ...
    for raw in env_path.read_text(encoding="utf-8").splitlines():
        ...
        key = key.strip()
        if key.startswith("AXIOM_") or key in ("QWEN_API_KEY", "DEEPSEEK_API_KEY", "GLM_API_KEY"):
